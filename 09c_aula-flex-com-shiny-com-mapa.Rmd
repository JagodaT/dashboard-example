...
df_aisp <- read_csv2("www/AispPorRegiao.csv")
df_cisp_aisp <- read_csv2("www/CorrespondenciaCispAisp.csv") %>%
  mutate_all(as.integer)
df_crimes <- read_rds("www/crimes.rds") %>%
...
#sf_deleg <- st_read("www/shape_limites_seg/lm_dp_2019.shp")
#sf_deleg_simple <- sf_deleg %>% rmapshaper::ms_simplify()
#sf_deleg_simple %>% #st_write("www/shape_limites_seg_simple/lm_dp_2019_simple.shp")
sf_deleg <- st_read("www/shape_limites_seg_simple/lm_dp_2019_simple.shp")
...
renderPlotly({
  ...
  p <- df_crimes_capital %>%
    ...
    coord_flip()  +
    labs(title="Top 4 AISP's por número de ocorrências", x="AISP")
